...
d3.layout.wordStream = function(){
    ...
    var cloudRadians = Math.PI / 180, toDegree = 180 / Math.PI,
        cw = calculateCanvasWidth(),
        ch = calculateCanvasHeight();

    function calculateCanvasWidth() {
        // Dynamic canvas width based on dataset size
        if (fileName && fileName.indexOf("Quantum") >= 0) {
            return 25000;
        }

        // For large datasets, calculate based on number of periods
        // Ensure at least 80 pixels per period for proper word placement
        if (data && data.length > 0) {
            var periodsCount = data.length;
            var minWidthPerPeriod = 80;
            var calculatedWidth = periodsCount * minWidthPerPeriod;

            // Cap at reasonable maximum (50000) to avoid memory issues
            var maxWidth = 50000;
            var minWidth = 1 << 14; // 16384 - original default
    ...
    function calculateCanvasHeight() {
        // Dynamic canvas height for large datasets to fit sprite generation
        // Larger datasets need more vertical space to lay out word sprites
        if (data && data.length > 200) {
            return 1 << 13; // 8192 for very large datasets
        } else if (data && data.length > 100) {
            return 1 << 12; // 4096 for large datasets
        }
    ...
    wordStream.boxes = function(){
        // Recalculate canvas dimensions based on actual data size
        cw = calculateCanvasWidth();
        ch = calculateCanvasHeight();

        var boxWidth = size[0]/data.length;

        // Log canvas and dataset info for debugging large datasets
        if (data.length > 100) {
            console.log('Large dataset detected:');
            console.log('  Periods:', data.length);
            console.log('  Canvas dimensions:', cw, 'x', ch);
            console.log('  Pixels per period (width):', (cw / data.length).toFixed(1));
            console.log('  Visualization width:', size[0]);
            console.log('  Box width per period:', boxWidth.toFixed(1));
        }
        ...
        // Log placement statistics for large datasets
        if (data.length > 100) {
            var totalWords = 0;
            var placedWords = 0;
            boxes.data.forEach(function(period) {
                boxes.topics.forEach(function(topic) {
                    if (period.words[topic]) {
                        period.words[topic].forEach(function(word) {
                            totalWords++;
                            if (word.placed) placedWords++;
                        });
                    }
                });
            });
            var placementRate = (placedWords / totalWords * 100).toFixed(1);
            console.log('Word placement statistics:');
            console.log('  Total words:', totalWords);
            console.log('  Successfully placed:', placedWords);
            console.log('  Failed to place:', (totalWords - placedWords));
            console.log('  Placement rate:', placementRate + '%');
            if (placementRate < 80) {
                console.warn('⚠️ Low placement rate detected! Try:');
                console.warn('  1. Reduce "Top" parameter to show fewer words per period');
                console.warn('  2. Increase visualization width');
                console.warn('  3. Reduce font size range');
            }
        }
        ...
        for(var i = 0; i < data.length; i++){
            boxes.topics.forEach(topic =>{
                ...
                while (++di < n) {
                    ...
                    if (y + h >= ch) {
                        console.warn('Sprite canvas overflow: Not all words could be rendered to sprites.',
                                    'Consider reducing "Top" parameter or canvas is too small for dataset size.');
                        break;
                    }
        ...
        for(var bc = 0; bc < data.length; bc++){
            boxes.topics.forEach(topic=>{
                ...
                while (++di < n) {
                    ...
                    // Validate dimensions are valid integers before getImageData
                    if (!isFinite(d.x) || !isFinite(d.y) || !isFinite(d.width) || !isFinite(d.height) ||
                        d.width <= 0 || d.height <= 0) {
                        console.warn('Skipping word with invalid dimensions:', d.text, {x: d.x, y: d.y, width: d.width, height: d.height});
                        continue;
                    }

                    var pixels = c.getImageData(Math.floor(d.x), Math.floor(d.y), Math.floor(d.width), Math.floor(d.height)).data;
                    d.sprite = Array();
