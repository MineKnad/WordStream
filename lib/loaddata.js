...
function getTop(data, topics, top){
    data.forEach((d) => {
        topics.forEach((topic) => {
            // Check if the topic exists and has words before slicing
            if (d["words"] && d["words"][topic] && d["words"][topic].length > 0) {
                d["words"][topic] = d["words"][topic]
                    .slice(0,top);
                d["words"][topic].sort(function(a, b){//sort the terms by frequency
                    return b.sudden-a.sudden})
            } else if (d["words"] && !d["words"][topic]) {
                // Initialize empty array if topic doesn't exist
                d["words"][topic] = [];
            }
        })
    });
...
/**
 * Validate which categories actually have data across all periods
 * @param {Array} data - Array of period objects with words
 * @param {Array} declaredCategories - Categories from metadata
 * @returns {Object} { validCategories, missingCategories, categoryStats }
 */
function validateAndFilterCategories(data, declaredCategories) {
    const categoryPresence = {};
    const totalPeriods = data.length;

    // Initialize counters
    declaredCategories.forEach(cat => {
        categoryPresence[cat] = { count: 0, totalWords: 0 };
    });

    // Count presence across all periods
    data.forEach((period) => {
        declaredCategories.forEach(cat => {
            if (period.words && period.words[cat] && period.words[cat].length > 0) {
                categoryPresence[cat].count++;
                categoryPresence[cat].totalWords += period.words[cat].length;
            }
        });
    });

    // Separate valid vs missing categories (10% presence threshold)
    const validCategories = [];
    const missingCategories = [];

    declaredCategories.forEach(cat => {
        const stats = categoryPresence[cat];
        const presenceRate = (stats.count / totalPeriods) * 100;

        if (stats.count > 0 && presenceRate >= 10) {
            validCategories.push(cat);
        } else {
            missingCategories.push({
                category: cat,
                presenceCount: stats.count,
    ...
    return { validCategories, missingCategories, totalPeriods };
}

/**
 * Remove invalid categories from data structure
 * @param {Array} data - Array of period objects
 * @param {Array} validCategories - Categories that have actual data
 * @returns {Array} Cleaned data with only valid categories
 */
function cleanupDataWithValidCategories(data, validCategories) {
    return data.map(period => {
        const cleanedWords = {};

        validCategories.forEach(cat => {
            if (period.words && period.words[cat]) {
                cleanedWords[cat] = period.words[cat];
            } else {
                // For valid categories, ensure empty array exists
                cleanedWords[cat] = [];
            }
        });

        return { ...period, words: cleanedWords };
    });
}
...
function processSudden(data){
    ...
    subjects.forEach((topic, i) => {
        for (var j = 1; j < data.length; j++) {
            // Check if topic exists and is array
            if (data[j]["words"][topic] && Array.isArray(data[j]["words"][topic])) {
                data[j]["words"][topic].forEach((word, k) => {
                    var prev = 0;
                    // Safely check previous period
                    if (data[j - 1]["words"][topic] && Array.isArray(data[j - 1]["words"][topic])) {
                        var prevWord = data[j - 1]["words"][topic].find(d => d.text === word.text);
                        if (prevWord && prevWord.frequency) {
                            prev = prevWord.frequency;
                        }
                    }
                    word.sudden = (word.frequency + 1) / (prev + 1);
                })
            } else if (!data[j]["words"][topic]) {
